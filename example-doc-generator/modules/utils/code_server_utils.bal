...
# Ensures a VS Code extension is installed in code-server from the marketplace.
# + extensionId - the extension identifier (e.g. "wso2.wso2-integrator")
# + return - an error if the install fails
public function ensureExtensionInstalled(string extensionId) returns error? {
    log("\t[INFO] Installing from marketplace: " + extensionId);
    os:Process|error proc = os:exec({
        value: "code-server",
        arguments: ["--install-extension", extensionId]
    });
    if proc is error {
        return error("Failed to launch extension install: " + proc.message());
    }
    int|error exitCode = proc.waitForExit();
    if exitCode is error {
        return error("Extension install process error: " + exitCode.message());
    }
    if exitCode != 0 {
        return error("Extension install from marketplace failed with exit code: " + exitCode.toString());
    }
}
